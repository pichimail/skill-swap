...
async function loadProfile(userId: string) {
  ...
  const skills = await sql`
    SELECT s.id, s.name, us.kind, us.level
...
export async function PATCH(request: Request) {
  try {
    ...
    await sql`
      UPDATE users SET
        display_name = CASE WHEN ${input.displayName !== undefined} THEN ${input.displayName ?? null} ELSE display_name END,
        ...
        timezone = CASE WHEN ${input.timezone !== undefined} THEN ${input.timezone ?? null} ELSE timezone END,
