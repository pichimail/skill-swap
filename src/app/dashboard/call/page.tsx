...
export default function CallPage() {
  ...
  useEffect(() => {
    ...
    return () => { void disconnectRoom(false); };
  }, []);
  ...
  return <div className="ss-page ss-page-pad !pb-3 min-h-[calc(100dvh-122px)] lg:min-h-[calc(100dvh-64px)] flex flex-col gap-4">
    ...
    <div className="flex-1 min-h-[420px] grid grid-rows-2 md:grid-rows-1 md:grid-cols-2 gap-2 lg:gap-3">
      <div className="relative overflow-hidden bg-black border border-white/10 rounded-[10px] min-h-[200px]"><video ref={remoteVideoRef} autoPlay playsInline className="absolute inset-0 w-full h-full object-cover" /><div className="absolute inset-0 -z-0 grid place-items-center text-white/35 text-sm">{state === 'connected' ? 'Waiting for remote video…' : 'Not connected'}</div><span className="absolute left-3 bottom-3 bg-black/70 text-white border border-white/10 px-3 py-1.5 rounded-full text-[11px] font-bold">{partner}</span></div>
      <div className="relative overflow-hidden bg-black border border-white/10 rounded-[10px] min-h-[200px]">{cameraOn ? <video ref={localVideoRef} autoPlay playsInline muted className="absolute inset-0 w-full h-full object-cover mirror-mode" /> : <div className="absolute inset-0 grid place-items-center text-white/35">Camera off</div>}<span className="absolute left-3 bottom-3 bg-black/70 text-white border border-white/10 px-3 py-1.5 rounded-full text-[11px] font-bold">You</span></div>
    </div>
